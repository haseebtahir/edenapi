			...
			if (companyExists(c.getCompanyID())) {
				...
				if (status == 1) {
					response = Response.build("Success",
							"company and all associated records (products and product photos)"
		...
		try {
			...
			if (c.getCustomerID() == 0) {
				...
				if (customerExists(c.getCustomerID())) {
					...
					response = Response.build("Success", "Customer ID " + c.getCustomerID()
						+ " has " + companyList.size() + " companies listed", true);
					response.setData(companyList);
					}
