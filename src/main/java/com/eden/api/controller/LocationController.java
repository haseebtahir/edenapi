...
public class LocationController extends BaseController {
		...
		try {
			...
			if (locValid.isNull(l) == true) {
				response = Response.build("Error",
						"mandatory parameters : [staff_email / num_max_rows / num_max_columns / locationName / rate ] cannot be null",
						false);
				return response;
			} // isNull check
			...
			if (locValid.fieldsAreEmpty(l) == true) {
				response = Response.build("Error",
						"no data entered for mandatory parameters, either [staff_email / num_max_rows / num_max_columns / locationName / rate ]",
						false);
				return response;
			} // isEmpty check
				...
				if (location_id > 0) {
					...
					for (i = 0; i < l.getRateList().size(); i++) {
						...
						if (zone_id > 0) {
							...
							// set the zone id to zoneList item
	...
	public @ResponseBody Response getLocation(@RequestBody CreateLocation l) {
		...
		try {
			...
			l.setLocationName(locationService.getLocation(l.getLocationID()).getLocationName());
